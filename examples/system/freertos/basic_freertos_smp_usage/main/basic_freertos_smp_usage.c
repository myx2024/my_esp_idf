...
static void config_console(void)
{
    ...
    register_creating_task();       //运行演示如何创建和运行固定和未固定任务的示例
    register_queue();               //运行演示如何使用队列在任务之间通信的示例
    register_lock();                //运行演示如何使用互斥锁和自旋锁来保护共享资源的示例
    register_task_notification();   //运行演示如何使用任务通知同步任务的示例
    register_batch_proc_example();  //运行将队列、互斥锁、任务通知组合在一起的示例
...
void app_main(void)
{
    config_console();//初始化和配置控制台 REPL 环境，允许用户通过命令行与系统交互
}
